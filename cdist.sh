...
while read l; do
	...
	x2x +sf < $inDir/ref.raw | frame -l 400 -p 160 | window -l 400 -L 512 | \
	    mcep -l 512 -m 25 -a 0.42 -e 0.000001 | x2x +fa26 | \
	    sed -n '20,180p' | x2x +af > $inDir/ref.mcep
	vstat -l 26 -o 1 $inDir/ref.mcep > $inDir/mean
	vopr -l 26 -s $inDir/ref.mcep $inDir/mean > $inDir/ref.n.mcep

	sox $voc $inDir/voc.raw
	x2x +sf < $inDir/voc.raw | frame -l 400 -p 160 | window -l 400 -L 512 | \
	    mcep -l 512 -m 25 -a 0.42 -e 0.000001 | x2x +fa26 | \
	    sed -n '20,180p' | x2x +af > $inDir/voc.mcep
